...
class FancOutputFiles:
    directory = os.getcwd()
    def save_tables(dataset, csv_file, xml_file_prefix):
        ...
        for day, data_rows in datasets_by_day.items():
            day_root = ET.Element("Workbook")
            list_date_elem = ET.SubElement(day_root, "ListDate")
            ...
            for table_name, times in time_groups.items():
                ...
                day_tree = ET.ElementTree(day_root)
                # Append index to xml_file_prefix to create separate files for each day
            create_date = document_properties["CreateDate"].replace("/", "")
            create_time = document_properties["CreateTime"].replace(":", "")
            xml_file_name_Transfer = f"{xml_file_prefix}_{day}_{create_date}_{create_time}.xml"
            day_tree.write(xml_file_name_Transfer, encoding='utf-8', xml_declaration=True)

            #xml_file_name = ""
            #FancSftp.sftp_upload('/home/avi/scripts_python/Gefen/gefenssh.key','le3srqnJ','nofrn#inova','snoga.noga-iso.co.il', FancOutputFiles.directory + "/send_report_xml/" + xml_file_name ,'/External Users BeforeScan/' + xml_file_name_Transfer)
            Fanc.PrintFile(" sftp_upload " + xml_file_name_Transfer ,"/EventFile.ini")
    ...
    def create_feport_files():
        ...
        if sql != "":
            ...
            xml_file_name = "NOFARN_Day_supplyplan" + days_
